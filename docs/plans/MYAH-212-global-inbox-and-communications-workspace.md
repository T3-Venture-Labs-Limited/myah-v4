...
- User-approved staging: MYAH-209 is rebased from `origin/main`. Until MYAH-210 merges, preserve this committed spec/plan history only; do not introduce MessageThread metadata, `1784266302005`, metadata GraphQL generation, database integration, Inbox server/frontend contracts, or runtime/browser-UAT claims. After MYAH-210 merges, rebase onto `origin/main` and complete Tasks 1–2 and their deferred integration gates. Do not start any Inbox production source for Tasks 3–7 until MYAH-210 is merged and Tasks 1–2 are complete; then run the full local UAT. MYAH-210 reserves `1784266302003` and `1784266302004`; MYAH-212 uses `1784266302005` and never edits or replays another committed command.
