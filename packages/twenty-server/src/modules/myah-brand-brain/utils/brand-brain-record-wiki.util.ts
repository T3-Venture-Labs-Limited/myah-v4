...
export const buildCanonicalPath = ({
  ...
  return normalizePath(
    [normalizedParentPath, normalizeSlug(slug)].filter(Boolean).join('/'),
  );
};
...
export const resolveBrandBrainPath = ({
  ...
  const page = pages.find(
    (candidatePage) =>
      normalizePath(candidatePage.canonicalPath) ===
...
export const resolveBrandBrainIdPath = ({
  ...
  const page = pages.find(
    (candidatePage) =>
      normalizePath(candidatePage.idPath ?? '') === normalizePath(idPath),
  );
  return page ? { page, ancestors: getAncestors({ page, pages }) } : null;
};
...
export const findDuplicateSiblingSlug = ({
  ...
  pages.find(
    (page) =>
      ...
      normalizeSlug(page.slug) === normalizeSlug(input.slug),
  ) ?? null;
export const findDuplicateCanonicalPath = ({
  ...
  pages.find(
    (page) =>
      normalizePath(page.canonicalPath) === normalizePath(input.canonicalPath),
  ) ?? null;
