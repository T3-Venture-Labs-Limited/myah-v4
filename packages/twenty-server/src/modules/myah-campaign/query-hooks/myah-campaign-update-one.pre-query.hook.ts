...
@WorkspaceQueryHook('campaign.updateOne')
export class MyahCampaignUpdateOnePreQueryHook implements WorkspacePreQueryHookInstance {
  constructor(
  ...
  validateRawInput(
    ...
    _objectName: string,
    payload: UpdateOneResolverArgs<CampaignMutationData>,
    context: WorkspaceRawInputPreQueryHookContext,
  ): void {
    this.campaignLifecycleService.validateRawUpdate(context, payload);
  }
  ...
  async execute(
    ...
    objectName: string,
    payload: UpdateOneResolverArgs<CampaignMutationData>,
  ): Promise<UpdateOneResolverArgs<CampaignMutationData>> {
    return this.campaignLifecycleService.validateStatusBearingUpdateOne(
      authContext,
      objectName,
      payload,
    );
