...
const { result, error, success } = await runAgent({
  ...
  operationId: event.id, // stable across retries of the same logical run
...
- `operationId` must uniquely identify the logical run and must be reused when
  retrying that run; a new logical run needs a new identifier.
